...
function geocodeLatLng(lat, lng, map, infowindow) {
  let clickedAddress
  let contentString = `<form onsubmit="onSetLocationName(event)" >
    <input name="nameinput" type="text" placeholder="Type Location's Name">
    </form>`
  const latlng = {
    lat,
    lng,
    ...
    .then((response) => {
      if (response.results[0]) {
        ...
        infowindow.setContent(
          response.results[0].formatted_address + contentString
        )
